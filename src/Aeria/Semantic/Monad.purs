...
checkArrayType expected property (VArray (v : vs)) = case arrayType (v : vs) of
  Just arrType -> when (arrType /= expected) (Left (PropertyError property $ ArrayTypeMismatch expected arrType))
  Nothing ->
    let
      received = valueType v
    in
      Left (PropertyError property (ArrayTypeMismatch expected received))
...
checkBoolean property v =
  let
    received = valueType v
  in
    Left (PropertyError property (TypeMismatch TBoolean received))
...
checkNumber property v =
  let
    expected = L.fromFoldable [ TFloat, TInteger ]
    received = valueType v
  in
    Left (PropertyError property (TypesMismatch expected received))
...
checkInteger property value =
  let
    received = valueType value
  in
    Left (PropertyError property (TypeMismatch TInteger received))
...
checkCollection = go
  ...
  extendContext properties getters ctx =
    let
      ctx' = extendProperties ctx properties
    in
      extendGetters ctx' getters

  extendProperties :: Context -> Properties -> Context
  extendProperties =
    L.foldl \(Context { properties, getters }) property@(Property { propertyName: (PropertyName name) }) ->
      let
        properties' = extend name property properties
      in
        (Context { properties: properties', getters })

  extendGetters :: Context -> Getters -> Context
  extendGetters =
    L.foldl \(Context { properties, getters }) getter@(Getter { getterName: (PropertyName name) }) ->
      let
        getters' = extend name getter getters
      in
        (Context { getters: getters', properties })
...
checkProperty property@(Property { propertyType }) =
  let
    validate = mkPropertyValidate propertyType
  in
    validate property

mkPropertyValidate :: PropertyType -> Property -> Either SemanticError Unit
mkPropertyValidate = case _ of
...
checkArrayProperty property@(Property { propertyType, propertyAttributes }) = case propertyType of
  ...
  validate (p : ps) = case checkProperty p of
    Right _ -> validate ps
    Left err -> Left err
...
checkObjectProperty property@(Property { propertyType, propertyAttributes })
  | L.length propertyAttributes > 0 = Left (PropertyError property UnexpectedAttributes)
  | otherwise = case propertyType of
...
checkStringProperty property = checkAttributes property validations
  ...
  checkFormat property' (VString format)
    ...
    | otherwise =
      let
        expected = L.fromFoldable formatOptions
      in
        Left (PropertyError property' (AttributeValueMustBe expected))
  checkFormat property' value =
    let
      received = valueType value
    in
      Left (PropertyError property' (TypeMismatch TString received))
  ...
  checkType property' (VString typ)
    ...
    | otherwise =
      let
        expected = L.fromFoldable typeOptions
      in
        Left (PropertyError property' (AttributeValueMustBe expected))
...
checkAttributes property@(Property { propertyAttributes }) validations = traverse_ check propertyAttributes
  where
  check :: Attribute -> Either SemanticError Unit
  check (Attribute attributeName@(Name name) value) = case M.lookup name validations of
